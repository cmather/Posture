define(["Posture/Class", "Posture/behaviors/EventPublisher"], function(Posture) {
  ...
  var Component = {
    ...
    configure: function configure(config) {
      ...
      this.componentChildren = this.children;
      this.configChildren = config.children;
      this.children = [];
    ...
    renderChildren: function renderChildren() {
      _.forEach(this.children, function(child) {
        this.renderChild(child, "append");
      }, this);
    }.posture("hooks", "renderChildren"),

    renderChild: function renderChild(child, method, index) {
      child.render(this.el, method, index);
      return this;
    ...
    processChild: function processChild(child, key, options) {
      var instance = this.addChildComponent(child, options);
      if (key) this[key] = instance;
      return instance;
    }.posture("hooks", "processChild"),

    addChildComponent: function addChildComponent(child, options) {
      var childrenDelegate = this, index;
      if (options && options.useDelegate) childrenDelegate = this.delegateFor("children");
      ...
      /* TODO: Allow an insertAt option or an index */

      if (options && (index = options.index)) {
        childrenDelegate.children.splice(index, 0, child);
      }
      else {
        childrenDelegate.children.push(child);
      }

      if (options && options.render) {
        child.render(childrenDelegate.el, "insert", index);
      }

      return child;

    }.posture("hooks", "addChildComponent"),
    ...
    childComponentFrom: function childComponentFrom(child, parent) {
      ...
      return instance;
    }.posture("hooks", "childComponentFrom"),
