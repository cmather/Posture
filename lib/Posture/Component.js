define(["Posture/Class", "Posture/behaviors/EventPublisher"], function(Posture) {
  ...
  var Component = {
    ...
    configure: function configure(config) {
      ...
      this.index = _.defaultValue(config.index, null);
      this.id = _.defaultValue(config.id, null);
    ...
    render: function render(container, index) {
      ...
      method = (_.isUndefined(index) || _.isNull(index)) ? "append" : "insert";
    ...
    addChild: function addChild(child, options) {
      ...
      childrenDelegate.addChildToRenderQueue(child);
    ...
    removeChild: function removeChild(child) {
      ...
      return false;
    }.posture("hooks", "removeChild"),

    addChildToRenderQueue: function addChildToRenderQueue(child) {
      if (!_.isUndefined(child.index) && child.index >= 0) {
        this.children.splice(child.index, 0, child);
      }
      else {
    ...
    childFactoryFrom: function childFactoryFrom(factory) {
      ...
      if (!_.classInheritsFrom(Posture.Component, result)) {
        throw new Error(this.type() + ": A child factory must inherit from Posture.Component and the provided factory - " + result.namespace() + " - does not.˝");
      }
